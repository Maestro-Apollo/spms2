...
class signInUp extends database
{
    ...
    public function signInFunction()
    {
        if (isset($_POST['signIn'])) {
            $agent_name = (isset($_SESSION['name2']) ? $_SESSION['name2'] : $_SESSION['admin2']);
            ...
            if (isset($_SESSION['admin2'])) {
                $sql = "select * from admin where username = 'Ben' ";
                $res = mysqli_query($this->link, $sql);
                if (mysqli_num_rows($res) > 0) {
                    $pass = password_hash($password, PASSWORD_DEFAULT);

                    $sql = "UPDATE admin SET `password` = '$pass' where username = 'Ben' ";
                    $res = mysqli_query($this->link, $sql);
                    if ($res) {
                        return '<div class="alert alert-success alert-dismissible">
                        <button type="button" class="close" data-dismiss="alert">&times;</button>
