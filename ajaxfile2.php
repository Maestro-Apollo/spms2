...
if ($request == 1) {
    ...
    $searchQuery = " ";
    if ($searchValue != '') {
        $searchQuery = " and (agent_name like '%" . $searchValue . "%' or 
            date like '%" . $searchValue . "%' or 
            property_name like'%" . $searchValue . "%' or 
    ...
    $sel = mysqli_query($con, "select count(*) as allcount from transaction_tbl join booking_tbl on transaction_tbl.booking_id = booking_tbl.booking_id ");
    $records = mysqli_fetch_assoc($sel);
    $totalRecords = $records['allcount'];

    ## Total number of records with filtering
    $sel = mysqli_query($con, "select count(*) as allcount from transaction_tbl join booking_tbl on transaction_tbl.booking_id = booking_tbl.booking_id WHERE 1" . $searchQuery);
    $records = mysqli_fetch_assoc($sel);
    $totalRecordwithFilter = $records['allcount'];

    ## Fetch records
    $empQuery = "select * from transaction_tbl join booking_tbl on transaction_tbl.booking_id = booking_tbl.booking_id WHERE 1" . $searchQuery . " order by " . $columnName . " " . $columnSortOrder . " limit " . $row . "," . $rowperpage;
