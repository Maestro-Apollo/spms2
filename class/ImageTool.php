...
class ImageTool {
    ...
    static function resizeWater($newWidth, $targetFile, $originalFile) {
        ...
        $transparent = imagecolorallocatealpha($newImg, 255, 255, 0, 0);
        $s = imagefilledrectangle($newImg, 0, 0, $newWidth, $newHeight, $transparent);
        imagecopyresampled($newImg, $im, 0, 0, 0, 0, $newWidth, $newHeight,
            $width, $height);
    ...
    public static function watermark($options = []) {
        ...
        if (!self::createPath($options['output'])) {
            static::$_errors[] = 'if output path (directories) doesnt exist, try to make whole path '.$options['output'];
            return false;
        }
        //debug($options);
        ...
        if (empty($img)) {
            static::$_errors[] = 'Not open image '.$options['input'];
            return false;
        }
        unset($options['input']);
        ...
        if (empty($src_wm)) {
            static::$_errors[] = 'Not open watermark '.$options['watermark'];
            return false;
        }
        ...
        if(! empty($options['wm_resize'])){

            $ratio_wm = $img_wm_w/$img_wm_h;
            $percentWater = intval($options['wm_resize']);
            $resize_img_wm_h = intval($img_im_h * $percentWater/100);

            $img_wm_h_r = $resize_img_wm_h;
            $img_wm_w_r = intval($ratio_wm * $img_wm_h_r);
            $fileInfoWater = pathinfo($watermarkPath);
            $resizeWaterOption = [
                'height' => $img_wm_h_r,
                'width' => $img_wm_w_r,
                'input' => $watermarkPath,
            ];
            $tmpName = $fileInfoWater['filename'].md5(json_encode($resizeWaterOption)).'.'.$fileInfoWater['extension'];
            $tmpWaterPath = str_replace( $fileInfoWater['basename'], $tmpName, $watermarkPath);

            if(! file_exists($tmpWaterPath)){
                $resizeWaterOption['output'] = $tmpWaterPath;

                $resultSize = self::resize($resizeWaterOption);
                if(! $resultSize){
                    throw new Exception('Error resize watermark '.json_encode($resizeWaterOption));
                }
            }
            $src_wm = self::openImage($tmpWaterPath);
            if(! $src_wm){
                throw new Exception('Error open image '.json_encode($resizeWaterOption).' '.$tmpWaterPath);
            }
            // $src_wm = $dst_im;
            $img_wm_w = $img_wm_w_r;
            $img_wm_h = $img_wm_h_r;
        ...
        if ($options['scale']) {
            ...
            switch ($options['position']) {
                ...
                case 'center':
                default:
                    $pos_x = round($img_im_w / 2 - $img_wm_w - ceil($img_im_w / 2 / $img_wm_w) - $img_wm_w / 2);
                    $pos_y = round($img_im_h / 2 - $img_wm_h - ceil($img_im_h / 2 / $img_wm_h) - $img_wm_h / 2);
        ...
        if (!$r) {
            static::$_errors[] = 'error append watermark';
            return false;
        }

        if (!self::afterCallbacks($img, $options['afterCallbacks'])) {
            static::$_errors[] = 'error afterCallbacks watermark';
            return false;
        }
     ...
     *
     * - src_x - Position X start crop
     * - src_y - Position Y start crop
     *
     * @param array $options An array of options
     * @return mixed boolean or GD resource if output was set to null
     */
    ...
    public static function openImage($input) {
        if (is_resource($input)) {
            ...
            if (is_string($input) && preg_match('/^https?:\/\//', $input)) {
                ...
                if (!$image) {
                    static::$_errors[] = 'empty content : '.$input;
                    return false;
                }

                return imagecreatefromstring($image);
            }
            $ext = self::getImageType($input);
            switch ($ext) {
                case 'jpg':
                ...
                default:
                    static::$_errors[] = 'Not Ext openImage : '.$ext;
                    break;
            }
        }
        ...
        if ($extension) {
            ...
            switch ($info['mime']) {
                ...
                case 'image/webp':
                    ...
                    break;
                default:
                    static::$_errors[] = 'Не поддерживает '.$info['mime'];
                    break;
            }
        }
    ...
    public static function saveImage(&$im, $options = []) {
        ...
        $saveType = self::getImageType($options['output'], true);
        switch ($saveType) {
            case 'jpg':
        ...
        unset($im);
        static::$_errors[] = 'not save '.$saveType. ' '.$options['output'];
        return false;
    }
...
//        echo "Path: ".print_r($arr_output_path, true)."\n";
